...
class MaskGenerator(DataProcessorBaseClass):
    ...
    def generate_enfusion_polylines(self):
        # Load the image from the file saved by VoronoiColorer
        img = Image.open(f'{self.save_path}/preview.png')
        data = np.array(img)
        ...
        # For each contour, create a polyline
        enfusion_utils = EnfusionUtils()
        polylines = []
        for contour in contours:
            # Convert the points to Enfusion format
            origin, relative_contour = self._convert_contour_to_relative_format(contour) 
            # Generate the Enfusion polyline
            enfusion_polyline = enfusion_utils.generate_enfusion_polyline(origin, relative_contour)
            polylines.append(enfusion_polyline)

        # Save the polylines to a file
        with open(f'{self.save_path}/polylines.layer', 'w') as f:
            for polyline in polylines:
                f.write(polyline + '\n')

    def _convert_contour_to_relative_format(self, contour):
        relative_contour = []
        points = contour.tolist()
        print(len(points))
        # Convert first point to global coordinates and invert y and x
        origin = [points[0][1], 0, self.svg_height - points[0][0]]
        #print(origin)
        # Convert remaining points to coordinates relative to origin
        relative_points = [[p[1] - origin[2], p[0] - origin[0]] for p in points[1:]]
        #print(relative_points[:5])
        # Add origin back to start of relative_points
        relative_contour.append(relative_points[0],0,relative_points[1])     
        return origin, relative_contour
    
    def generate_polylines(self):
        
        # Load the image in grayscale
        image = cv2.imread('preview.png', cv2.IMREAD_GRAYSCALE)

        # Threshold the image to isolate the black borders
        _, thresholded = cv2.threshold(image, 1, 255, cv2.THRESH_BINARY_INV)

        # Find the contours of the borders
        contours, _ = cv2.findContours(thresholded, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        # For each contour, convert it to a polyline
